...
//#define ASAP
//#define ILP_PEAKPARETO // min peak pareto
//#define ILP_PACMPARETO // pareto with peak=lee
//#define ILP_COSTPARETO // min cost pareto
#define LEE
#define PACMFG
//#define PACMGG
//#define PACMGF
...
int execute()
{
  ...
  */
  ...
  // print Ptot for gnuplot
  Signal<double>* P_tot = P__tot(*minPeakTab, *(sp->J()) );
  P_tot->setName("Ptot_Lee");
  P_tot->printIntoFile();
  ...
  // print Ptot for gnuplot
  Signal<double>* P_tot2 = P__tot(*minPeakTab2, *(sp->J()) );
  stringstream ss;
  ss << "Ptot_PACMFG_wLeePmax" << "_peakconstraint_" << pr.minPeakLee;
  P_tot2->setName(ss.str());
  P_tot2->printIntoFile();
  ...
  if(pr.isFeasiblePACMFG_costmin)
    {
      ...
      // print Ptot for gnuplot
      Signal<double>* P_tot6_1 = P__tot(*minCostTab6_1, *(sp->J()) );
      stringstream ss6_1;
      ss6_1 << "Ptot_PACMFG_nopeakconstraint";
      P_tot6_1->setName(ss6_1.str());
      P_tot6_1->printIntoFile();
    }
  else
  ...
  */
  ...
  ////////////////////////////////////////////////////////////////
  // Additional results with PACMFG for different peak constraints
  // for animation purposes.
  ////////////////////////////////////////////////////////////////
  //  double peakConstraint_step_animation_1 = pr.minPeakPACMFG_costmin - (pr.minPeakPACMFG_costmin - pr.minPeakLee)/4;
  //double peakConstraint_step_animation_2 = pr.minPeakPACMFG_costmin - 2 * (pr.minPeakPACMFG_costmin - pr.minPeakLee)/4;
  //double peakConstraint_step_animation_3 = pr.minPeakPACMFG_costmin - 3 * (pr.minPeakPACMFG_costmin - pr.minPeakLee)/4;

  double peakConstraint_step_animation_1 = 10.765315;
  double peakConstraint_step_animation_2 = 9.35053;
  double peakConstraint_step_animation_3 = 7.935745;
  ...
  ////////////////////////////////////////////////////////////////////
  TNondominatedSet* pNondominatedSet2_1 = new TListSet < SchedulingMOMHSolution >;
  vector<Signal<int>*> emptyTab2_1;
	
  // PACMFG: Cost minimization with Full tree search for NP jobs and peak to min price greedy heuristic for P jobs.
  // Sort jobs by Energy / PeakValue / Preemption
  sort(sp->J()->begin(), sp->J()->end(), sortTasksViaPreemptionAndFreedom); // sortTasksViaPreemption sortTasksViaPeakValue sortTasksViaEnergy sortTasksViaPreemptionAndPeak
  sp->setPMax( peakConstraint_step_animation_1 );

  double mPeak2_1 = numeric_limits<double>::max();
  double mCost2_1 = numeric_limits<double>::max();
  // Peak minTab 
  vector<Signal<int>*>* minPeakTab2_1 = NULL;

  //  try {
  allocTab( NP_COST_MIN_WITH_TREE_SEARCH, P_COST_MIN_WITH_TREE_SEARCH3, *(sp->J()), emptyTab2_1, pNondominatedSet2_1, sp->PMax(), *(sp->pMin()), *(sp->PH()) );
  //allocTab( NP_COST_MIN_WITH_TREE_SEARCH, P_COST_MINIMIZATION, *(sp->J()), emptyTab2, pNondominatedSet2, sp->PMax(), *(sp->pMin()), *(sp->PH()) );
  ...
  if (pNondominatedSet2_1->iSetSize == 0)
    {
      pr.isFeasiblePACMFG_wLee_Pmax = false;
      //cout << "No feasible schedule found!" << endl;
    }
  ...
  for(std::vector<TSolution*>::iterator it=pNondominatedSet2_1->begin(); it != pNondominatedSet2_1->end(); it++)
    {
      SchedulingMOMHSolution* msIt = (SchedulingMOMHSolution*) *it;
      //cout << "Pareto\t" << *msIt << endl;
      //printTab( *(msIt->getTab()) );
	...
	//minCostTab = msIt->getTab();
      ...
      if (mPeak2_1 > msIt->getPeak())
	{
	  mPeak2_1 = msIt->getPeak();
	  mCost2_1 = msIt->getCost();
	  minPeakTab2_1 = msIt->getTab();
	}  	    
      //delete msIt;
    }
  pr.minPeakPACMFG_wLee_Pmax = mPeak2_1;
  pr.minCostPACMFG_wLee_Pmax = mCost2_1;
  
  // print Ptot for gnuplot
  Signal<double>* P_tot2_1 = P__tot(*minPeakTab2_1, *(sp->J()) );
  stringstream ss_1;
  ss_1 << "Ptot_PACMFG_wStepAnimation_1_Pmax" << "_peakconstraint_" << peakConstraint_step_animation_1;
  P_tot2_1->setName(ss_1.str());
  P_tot2_1->printIntoFile();
  ...
  */
  ...
  if(pr.isFeasiblePACMFG_wLee_Pmax)
    {
      foutRes << "Min. peak: " << pr.minPeakPACMFG_wLee_Pmax << endl;
      foutRes << "Min. cost: " << pr.minCostPACMFG_wLee_Pmax << endl;
      printSchedule(foutRes, *(sp->J()), *minPeakTab2_1);
    }
  else
    foutRes << "Infeasible solution by PACMFG with Pmax = Lee!" << endl;
  foutRes << endl;
  ...
  // delete solutions. not needed because delete msIt above already deletes them.
  pNondominatedSet2_1->DeleteAll();
  delete pNondominatedSet2_1;
  ////////////////////////////////////////////////////////////////////
  // End of PACMFG - STEP ANIMATION 1 ////////////////////////////////
  ////////////////////////////////////////////////////////////////////
#endif
  ...
  // Sort jobs by Energy / PeakValue / Preemption
  sort(sp->J()->begin(), sp->J()->end(), sortTasksViaPreemptionAndFreedom); // sortTasksViaPreemption sortTasksViaPeakValue sortTasksViaEnergy sortTasksViaPreemptionAndPeak
  sp->setPMax( peakConstraint_step_animation_2 );
  ...
  //  try {
  allocTab( NP_COST_MIN_WITH_TREE_SEARCH, P_COST_MIN_WITH_TREE_SEARCH3, *(sp->J()), emptyTab2_2, pNondominatedSet2_2, sp->PMax(), *(sp->pMin()), *(sp->PH()) );
  //allocTab( NP_COST_MIN_WITH_TREE_SEARCH, P_COST_MINIMIZATION, *(sp->J()), emptyTab2, pNondominatedSet2, sp->PMax(), *(sp->pMin()), *(sp->PH()) );
  ...
  pr.minPeakPACMFG_wLee_Pmax = mPeak2_2;
  pr.minCostPACMFG_wLee_Pmax = mCost2_2;
  
  // print Ptot for gnuplot
  Signal<double>* P_tot2_2 = P__tot(*minPeakTab2_2, *(sp->J()) );
  stringstream ss_2;
  ss_2 << "Ptot_PACMFG_wStepAnimation_2_Pmax" << "_peakconstraint_" << peakConstraint_step_animation_2;
  P_tot2_2->setName(ss_2.str());
  P_tot2_2->printIntoFile();
  ...
  */
  ...
  delete pNondominatedSet2_2;
  ////////////////////////////////////////////////////////////////////
  // End of PACMFG - STEP ANIMATION 2 ////////////////////////////////
  ////////////////////////////////////////////////////////////////////
#endif
  ...
  // Sort jobs by Energy / PeakValue / Preemption
  sort(sp->J()->begin(), sp->J()->end(), sortTasksViaPreemptionAndFreedom); // sortTasksViaPreemption sortTasksViaPeakValue sortTasksViaEnergy sortTasksViaPreemptionAndPeak
  sp->setPMax( peakConstraint_step_animation_3 );
  ...
  //  try {
  allocTab( NP_COST_MIN_WITH_TREE_SEARCH, P_COST_MIN_WITH_TREE_SEARCH3, *(sp->J()), emptyTab2_3, pNondominatedSet2_3, sp->PMax(), *(sp->pMin()), *(sp->PH()) );
  //allocTab( NP_COST_MIN_WITH_TREE_SEARCH, P_COST_MINIMIZATION, *(sp->J()), emptyTab2, pNondominatedSet2, sp->PMax(), *(sp->pMin()), *(sp->PH()) );
  ...
  pr.minPeakPACMFG_wLee_Pmax = mPeak2_3;
  pr.minCostPACMFG_wLee_Pmax = mCost2_3;
  
  // print Ptot for gnuplot
  Signal<double>* P_tot2_3 = P__tot(*minPeakTab2_3, *(sp->J()) );
  stringstream ss_3;
  ss_3 << "Ptot_PACMFG_wStepAnimation_3_Pmax" << "_peakconstraint_" << peakConstraint_step_animation_3;
  P_tot2_3->setName(ss_3.str());
  P_tot2_3->printIntoFile();
  ...
  */
  ...
  delete pNondominatedSet2_3;
  ////////////////////////////////////////////////////////////////////
  // End of PACMFG - STEP ANIMATION 3 ////////////////////////////////
  ////////////////////////////////////////////////////////////////////
#endif
